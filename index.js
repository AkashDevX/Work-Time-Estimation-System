...
app.get("/tasks", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.user.role === "PM") {
      ...
      tasks = await prisma.task.findMany({ where: { userId: req.user.userId } });
    }
    res.json(tasks);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post("/tasks/:id/calculate-end", authMiddleware, async (req, res) => {
  try {
    ...
    const recurringHolidays = (await prisma.recurringHoliday.findMany()).map(h => ({
      day: h.day,
      month: h.month,
    }));

    const oneTimeHolidays = (await prisma.oneTimeHoliday.findMany()).map(h => ({
